    ...
    public String toString(int used) {
        ...
        for (String name : this.list.keySet()) {
            ...
            if (v instanceof MultiValue) {
                ...
                String ns = String.valueOf(name) + Marker.ANY_MARKER;
                for (int i = 0; i < vv.size(); i++) {
                    Object va = vv.get(i);
                    if (va instanceof Value) {
                        sb.addNV(String.valueOf(ns) + i + Marker.ANY_MARKER, ((Value) va).encodedValue);
                    } else {
                        sb.addNV(String.valueOf(ns) + i, (String) va);
                    }
                }
            } else if (v instanceof Value) {
                sb.addNV(String.valueOf(name) + Marker.ANY_MARKER, ((Value) v).encodedValue);
            } else {
