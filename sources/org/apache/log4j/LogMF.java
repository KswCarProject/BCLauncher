...
public final class LogMF extends LogXF {
        ...
        if (isSimple(pattern)) {
            ...
            while (pos >= 0) {
                if (pos + 2 >= pattern.length() || pattern.charAt(pos + 2) != '}' || pattern.charAt(pos + 1) < '0' || pattern.charAt(pos + 1) > '9') {
                    ...
                    retval = new StringBuffer().append(retval2).append(formatted[index]).toString();
                    prev = pos + 3;
        ...
        if (isSimple(pattern)) {
            ...
            while (pos >= 0) {
                if (pos + 2 >= pattern.length() || pattern.charAt(pos + 2) != '}' || pattern.charAt(pos + 1) < '0' || pattern.charAt(pos + 1) > '9') {
                    ...
                    String retval2 = new StringBuffer().append(retval).append(pattern.substring(prev, pos)).toString();
                    if (index != 0) {
                        retval = new StringBuffer().append(retval2).append(pattern.substring(pos, pos + 3)).toString();
                    } else {
                        if (formatted == null) {
                            formatted = formatObject(arg0);
                        }
                        retval = new StringBuffer().append(retval2).append(formatted).toString();
                    }
                    prev = pos + 3;
                    pos = pattern.indexOf(123, prev);
                }
    ...
    public static void trace(Logger logger, String pattern, boolean argument) {
        if (logger.isEnabledFor(TRACE)) {
            forcedLog(logger, TRACE, format(pattern, (Object) valueOf(argument)));
        }
    }
    ...
    public static void debug(Logger logger, String pattern, boolean argument) {
        if (logger.isDebugEnabled()) {
            forcedLog(logger, Level.DEBUG, format(pattern, (Object) valueOf(argument)));
        }
    }
    ...
    public static void info(Logger logger, String pattern, boolean argument) {
        if (logger.isInfoEnabled()) {
            forcedLog(logger, Level.INFO, format(pattern, (Object) valueOf(argument)));
        }
    }
    ...
    public static void warn(Logger logger, String pattern, boolean argument) {
        if (logger.isEnabledFor(Level.WARN)) {
            forcedLog(logger, Level.WARN, format(pattern, (Object) valueOf(argument)));
        }
    }
