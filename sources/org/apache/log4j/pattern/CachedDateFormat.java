...
public final class CachedDateFormat extends DateFormat {
    ...
    public CachedDateFormat(DateFormat dateFormat, int expiration2) {
        if (dateFormat == null) {
            throw new IllegalArgumentException("dateFormat cannot be null");
        } else if (expiration2 < 0) {
    ...
    public static int findMillisecondStart(long time, String formatted, DateFormat formatter2) {
        long slotBegin2 = (time / 1000) * 1000;
        if (slotBegin2 > time) {
            slotBegin2 -= 1000;
        }
        int millis = (int) (time - slotBegin2);
        int magic = MAGIC1;
        ...
        String plusMagic = formatter2.format(new Date(((long) magic) + slotBegin2));
        ...
        for (int i = 0; i < formatted.length(); i++) {
            if (formatted.charAt(i) != plusMagic.charAt(i)) {
                ...
                String plusZero = formatter2.format(new Date(slotBegin2));
                if (plusZero.length() == formatted.length() && magicString.regionMatches(0, plusMagic, i, magicString.length())) {
                    if (!formattedMillis.toString().regionMatches(0, formatted, i, magicString.length()) || !ZERO_STRING.regionMatches(0, plusZero, i, ZERO_STRING.length())) {
                        return -1;
                    }
                    return i;
                }
    ...
    public StringBuffer format(long now, StringBuffer buf) {
        ...
        } else if (this.millisecondStart == -1 || now >= this.slotBegin + ((long) this.expiration) || now < this.slotBegin || now >= this.slotBegin + 1000) {
