...
public abstract class AbstractDynamicMBean implements DynamicMBean, MBeanRegistration {
    ...
    public ObjectName preRegister(MBeanServer server2, ObjectName name) {
        getLogger().debug(new StringBuffer().append("preRegister called. Server=").append(server2).append(", name=").append(name).toString());
        this.server = server2;
        return name;
    }
