...
public class SyslogAppender extends AppenderSkeleton {
    ...
    public SyslogAppender(Layout layout, int syslogFacility2) {
        this.syslogFacility = 8;
        this.facilityPrinting = false;
        this.header = false;
        this.dateFormat = new SimpleDateFormat("MMM dd HH:mm:ss ", Locale.ENGLISH);
    ...
    public SyslogAppender(Layout layout, String syslogHost2, int syslogFacility2) {
        this(layout, syslogFacility2);
        setSyslogHost(syslogHost2);
    }
    ...
    public static String getFacilityString(int syslogFacility2) {
        switch (syslogFacility2) {
            case 0:
                return "kern";
            case 8:
                return "user";
    ...
    private void splitPacket(String header2, String packet) {
        if (packet.getBytes().length <= 1019) {
            this.sqw.write(packet);
            return;
        }
        int split = header2.length() + ((packet.length() - header2.length()) / 2);
        splitPacket(header2, new StringBuffer().append(packet.substring(0, split)).append("...").toString());
        splitPacket(header2, new StringBuffer().append(header2).append("...").append(packet.substring(split)).toString());
    }
    ...
    public void append(LoggingEvent event) {
        ...
        if (isAsSevereAsThreshold(event.getLevel())) {
            ...
            if ((this.layout == null || this.layout.ignoresThrowable()) && (s = event.getThrowableStrRep()) != null) {
    ...
    public void setSyslogHost(String syslogHost2) {
        this.sqw = new SyslogQuietWriter(new SyslogWriter(syslogHost2), this.syslogFacility, this.errorHandler);
        this.syslogHost = syslogHost2;
    }
