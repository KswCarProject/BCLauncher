...
public class SocketAppender extends AppenderSkeleton {
    ...
    public SocketAppender(InetAddress address2, int port2) {
        this.address = address2;
        this.remoteHost = address2.getHostName();
        this.port = port2;
        connect(address2, port2);
    }

    public SocketAppender(String host, int port2) {
        this.port = port2;
        this.address = getAddressByName(host);
        this.remoteHost = host;
        connect(this.address, port2);
    }
    ...
    public void connect(InetAddress address2, int port2) {
        String msg;
        if (this.address != null) {
            try {
                cleanUp();
                this.oos = new ObjectOutputStream(new Socket(address2, port2).getOutputStream());
            } catch (IOException e) {
                if (e instanceof InterruptedIOException) {
                    Thread.currentThread().interrupt();
                }
                String msg2 = new StringBuffer().append("Could not connect to remote log4j server at [").append(address2.getHostName()).append("].").toString();
            ...
            while (!this.interrupted) {
                try {
                    ...
                    synchronized (this) {
                        ...
                        SocketAppender.access$002(this.this$0, (Connector) null);
